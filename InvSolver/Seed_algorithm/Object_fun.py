...
class Obj_fun_Tools:
    ...
    def objective_fun(self,x,seed_js,G,d,derrs,refs,bounds,distance,ancestors_seeds_js,issmooth,seeds_neighbor_js,data_tool:data_tools,need_bound=False):
        _fun_x=copy.copy(x)
        x=np.array([x]).T
        punishment_diff_value_multiple=self.setting.punishment_diff_value_multiple
        ...
        for i in range(len(x)):
            ...
            if distance>1:
                res+=(self.setting.punishment_Search_distancevalue_multiple*distance**2+punishment_diff_value_multiple)*diff_value**2
            else: 
                res+=0
            ...
            if issmooth:
                res+=(x[i]- data_tool.get_value(ancestors_seeds_js[i]))**2*self.setting.prompt_value_multiple
            
            #目标函数增加平滑性
            if issmooth:
                for seeds_neighbor_j in  seeds_neighbor_js[i]:
                    res+=self.setting.smooth_all_multiple*(x[i]-refs[i]-(data_tool.get_value(seeds_neighbor_j)-data_tool.get_refs_value(seeds_neighbor_j)))**2
    ...
    def objective_gun(self,x,seed_js,G,d,derrs,refs,bounds,distance,ancestors_seeds_js,issmooth,seeds_neighbor_js,data_tool,need_bound=False):
        ...
        for i in range(len(x)):
            ...
            if distance>1:
                # punishment_diff_value_multiple /= (0.99 ** distance)
                res[i]+=(self.setting.punishment_Search_distancevalue_multiple*distance**2+self.setting.punishment_diff_value_multiple)*diff_value*2
            else: 
                res[i]+=0
            #目标函数增加促进该种子密度接近其祖先种子密度的约束
            if issmooth:
                res[i]+=(x[i]-data_tool.get_value(ancestors_seeds_js[i]))*2*self.setting.prompt_value_multiple
            #目标函数增加平滑性
            if issmooth:
                for seeds_neighbor_j in  seeds_neighbor_js[i]:
                    res[i]+=self.setting.smooth_all_multiple*(x[i]-refs[i]-(data_tool.get_value(seeds_neighbor_j)-data_tool.get_refs_value(seeds_neighbor_j)))*2
           #使用激活函数增加上下限约束
            if need_bound:
                if x[i]>=bounds[i][0] and x[i]<=bounds[i][1]:
                    res[i]+=0
